...
async function analyze({ dir, reactProductionProfiling = false, noMangling = false, appDirOnly = false, serve = false, port = 4000 }) {
    try {
        ...
        await (0, _promises.cp)(_nodepath.join(__dirname, '../../bundle-analyzer'), _nodepath.join(dir, ANALYZE_PATH), {
            recursive: true
        });
        // Collect and write routes for the bundle analyzer
        const routes = await collectRoutesForAnalyze(dir, config, appDirOnly);
        await (0, _promises.mkdir)(_nodepath.join(dir, ANALYZE_PATH, 'data'), {
            recursive: true
        });
        await (0, _promises.writeFile)(_nodepath.join(dir, ANALYZE_PATH, 'data', 'routes.json'), JSON.stringify(routes, null, 2));
        ...
        if (serve) {
            await startServer(_nodepath.join(dir, ANALYZE_PATH), port);
        }
    } catch (e) {
