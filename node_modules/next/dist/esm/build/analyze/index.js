...
export default async function analyze({ dir, reactProductionProfiling = false, noMangling = false, appDirOnly = false, serve = false, port = 4000 }) {
    try {
        ...
        await cp(path.join(__dirname, '../../bundle-analyzer'), path.join(dir, ANALYZE_PATH), {
            recursive: true
        });
        // Collect and write routes for the bundle analyzer
        const routes = await collectRoutesForAnalyze(dir, config, appDirOnly);
        await mkdir(path.join(dir, ANALYZE_PATH, 'data'), {
            recursive: true
        });
        await writeFile(path.join(dir, ANALYZE_PATH, 'data', 'routes.json'), JSON.stringify(routes, null, 2));
