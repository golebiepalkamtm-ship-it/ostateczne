# PowerShell script to configure Vercel with Supabase PostgreSQL
# Connection string format: postgresql://postgres:[PASSWORD]@db.fodfctgqzcuhqwcpupni.supabase.co:5432/postgres

Write-Host "Konfiguracja Vercel + Supabase PostgreSQL" -ForegroundColor Cyan
Write-Host ""

# Connection string do Supabase PostgreSQL
# Hasło: Milosz.1205 (kropka musi być URL-encoded jako %2E)
$DATABASE_URL_DIRECT = "postgresql://postgres:Milosz%2E1205@db.fodfctgqzcuhqwcpupni.supabase.co:5432/postgres"
$DATABASE_URL_PGBOUNCER = "postgresql://postgres:Milosz%2E1205@db.fodfctgqzcuhqwcpupni.supabase.co:6543/postgres?pgbouncer=true&connection_limit=1"

Write-Host "Connection strings:" -ForegroundColor Yellow
Write-Host "  Direct: $DATABASE_URL_DIRECT" -ForegroundColor Gray
Write-Host "  PgBouncer: $DATABASE_URL_PGBOUNCER" -ForegroundColor Gray
Write-Host ""
...
# 1. DATABASE_URL - Supabase PostgreSQL (z PgBouncer dla Vercel)
Write-Host "1. Konfiguracja bazy danych (Supabase PostgreSQL)..." -ForegroundColor Cyan
Write-Host "   Uzywam PgBouncer connection string dla Vercel (lepsze dla serverless)" -ForegroundColor Yellow
Add-VercelEnvVar "DATABASE_URL" $DATABASE_URL_PGBOUNCER "production"
Add-VercelEnvVar "DATABASE_URL" $DATABASE_URL_PGBOUNCER "preview"
Add-VercelEnvVar "DATABASE_URL" $DATABASE_URL_DIRECT "development"
Write-Host ""
...
Write-Host "  1. Uruchom migracje Prisma lokalnie: npm run db:migrate" -ForegroundColor Yellow
Write-Host "  2. Zdeployuj na Vercel: vercel --prod" -ForegroundColor Yellow
Write-Host ""
Write-Host "UWAGA: Dla migracji uzyj bezposredniego connection string (bez pgbouncer)" -ForegroundColor Yellow
Write-Host "      DATABASE_URL=$DATABASE_URL_DIRECT" -ForegroundColor Gray
Write-Host ""
